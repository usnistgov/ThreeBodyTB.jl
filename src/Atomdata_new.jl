...
#atoms["Is"] = makeatom("Si", 14, 3, 4, 28.09,  4.0,   0,   [:s, :p], -9.15243175, [-10.6594, -3.9477],0.007075865418, 0.5300262399999837);
    ...
    atoms["B" ] = makeatom("B" , 5, 2, 3, 10.81,    3.0,   0,   [:s, :p], -5.84632720, [-0.6875808938239544; -0.2613131797530708], 0.0, 0.6267943641604508, adj= 67.62585046150113  );
    atoms["C" ] = makeatom("C" , 6, 2, 4, 12.011,   4.0,   0,   [:s, :p], -10.78129974, [-1.002601890640717; -0.3846551855250298],0.0, 0.7978153170555154, adj= 60.516015221894946  );
    ...
    atoms["Al"] = makeatom("Al", 13, 3, 3, 26.98,  3.0,   0,   [:s, :p],  -6.37406439, [-0.5669635647556415; -0.19757116543256825], 0.0, 0.47560947595799796 , adj= 120.35327370972065  );
    atoms["Si"] = makeatom("Si", 14, 3, 4, 28.09,  4.0,   0,   [:s, :p], -9.15243181, [-0.7914241887679531; -0.2981179964824184],0.0, 0.5597222300055447, adj= 124.19186186707114  );
...
function prepare_atom(A, U, uni)    
    ...
    atoms[String(A) ] =  makeatom(String(A) ,old.Z, old.row, old.col, old.mass,   old.nval,   old.nsemicore,
                                  old.orbitals, old.total_energy, [old.eigs[o] for o in old.orbitals],     0.0 ,U, adj=old.voladj, semicore_orbitals=old.semicore_orbitals, semicore_eigs=[old.semicore_eigs[o] for o in old.semicore_orbitals])
