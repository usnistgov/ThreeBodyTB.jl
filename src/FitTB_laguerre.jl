...
#using ..CalcTB:n_2body_default                 
#using ..CalcTB:n_2body_onsite_default          
#using ..CalcTB:n_2body_S_default               
#using ..CalcTB:n_3body_default                 
#using ..CalcTB:n_3body_same_default            
#using ..CalcTB:n_3body_triple_default          
#using ..CalcTB:n_3body_onsite_default          
#using ..CalcTB:n_3body_onsite_same_default     
#using ..CalcTB:n_eam_default                   

using ..CalcTB:fitting_version_default
...
function do_fitting(list_of_tbcs; dft_list = missing, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, fit_eam=false, fit_pert=false, fitting_version=fitting_version_default)

    ret = do_fitting_linear(list_of_tbcs; dft_list=dft_list, fit_threebody=fit_threebody, fit_threebody_onsite = fit_threebody_onsite, do_plot = do_plot, fit_eam=fit_eam, fit_pert=fit_pert, fitting_version=fitting_version)
    return ret[1]

end

function generate_additional_hams(TOTX ,  TOTY, threebody_inds, ch_final; lam = [0.0,0.0])
...
#=    
    ...
    s = collect(shuffle(1: size(TOTX)[1]))
    P = collect(partition( s, Int64(ceil( length(s)/5))))

    for p in P
        println("p $p")
        ch_all = [TOTX[p, :]; TOTX[end - ninds+1:end, :]] \ [TOTY[p]; TOTY[end - ninds+1:end]]
        push!(CH, ch_all)
...
"""
function prepare_for_fitting(list_of_tbcs; kpoints = missing, dft_list = missing, fit_threebody=false, fit_threebody_onsite=false, starting_database=missing, refit_database=missing, fit_eam=false, fit_pert=false, fitting_version = fitting_version_default)
...
#        println(tbc)
        ...
        @time twobody_arrays, threebody_arrays, hvec, svec, Rvec, INDvec, h_onsite, ind_convert, dmin_types, dmin_types3 =  calc_tb_prepare_fast(tbc, use_eam=fit_eam, use_pert = fit_pert, use_threebody=fit_threebody, use_threebody_onsite=fit_threebody_onsite, spin = 1, fitting_version = fitting_version)
        if tbc.nspin == 2
            @time twobody_arrays, threebody_arrays, hvec2, svec, Rvec, INDvec, h_onsite, ind_convert, dmin_types, dmin_types3 =  calc_tb_prepare_fast(tbc, use_eam=fit_eam, use_pert = fit_pert, use_threebody=fit_threebody, use_threebody_onsite=fit_threebody_onsite, spin = 2, fitting_version=fitting_version)
        end
...
#                    println("HIND ", (key,3), " ", 1+hnum:hnum+threebody_arrays[key][2].sizeH)
    ...
    println("final threebody_inds $threebody_inds")
...
#    println("A error S: ", sum((X_S[:, :] * cs2  - Y_S[:]).^2))
    ...
    flush(stdout)
    sleep(0.001)
#    println("S 0.ones test ", sum(abs.(X_Snew_BIG * (0.1*ones(size(cs))) - (Y_Snew_BIG  - Xc_Snew_BIG))))
#    X_Snew_BIG = []
...
function do_fitting_linear(list_of_tbcs; kpoints = missing, dft_list = missing,  fit_threebody=true, fit_threebody_onsite=true, do_plot = false, starting_database=missing, mode=:kspace, return_database=true, NLIM=120, refit_database=missing, fit_eam=false, ch_startX = missing, fit_pert= false, lam=1e-8, fitting_version=fitting_version_default)
    ...
    X_Hnew_BIG, Y_Hnew_BIG, X_H, X_S, X_Snew_BIG, Y_Snew_BIG,  Y_H, Y_S, Xc_Hnew_BIG, Xc_Snew_BIG, HON, ind_BIG, KEYS, HIND, SIND, DMIN_TYPES, DMIN_TYPES3, keepind, keepdata, YS_new, cs, ch_refit, SPIN, threebody_inds  =
        prepare_for_fitting(list_of_tbcs; kpoints = kpoints,dft_list=dft_list, fit_threebody=fit_threebody, fit_threebody_onsite=fit_threebody_onsite,
                            starting_database=starting_database, refit_database=refit_database, fit_eam=fit_eam, fit_pert=fit_pert, fitting_version=fitting_version)
    ...
    if mode == :rspace
        ...
        if !ismissing(ch_startX)
            println("using ch_startX linear rspace")
            n = length(ch_startX)
            if n == size(X_H)[1]
                ch = ch_startX
            else
                t = X_H[:,1:n] * ch_startX
                @time ch_t = X_H[n+1:end,:] \ (Y_H - t)
                ch = vcat(ch_startX,ch_t)
            end
...
#        println("--")
    ...
    elseif mode == :kspace
        ...
        if lam > 1e-10
            regulariz = collect(I(ncols)) * lam

            X_Hnew_BIG_2 = [X_Hnew_BIG[rind,:] ; regulariz]
            Y_Hnew_BIG_2 = [Y_Hnew_BIG[rind] ; zeros(ncols)]
            Xc_Hnew_BIG_2 = [Xc_Hnew_BIG[rind] ; zeros(ncols)]
        else
            X_Hnew_BIG_2 = X_Hnew_BIG[rind,:] 
            Y_Hnew_BIG_2 = Y_Hnew_BIG[rind]
            Xc_Hnew_BIG_2 = Xc_Hnew_BIG[rind]
        end
            

        if !ismissing(ch_startX)
            println("using ch_startX linear kspace")
            n = length(ch_startX)
            if n == size(X_Hnew_BIG)[1]
                ch = ch_startX
            else
                t = X_Hnew_BIG_2[:,1:n] * ch_startX
                @time ch_t = X_Hnew_BIG_2[:, n+1:end] \ (Y_Hnew_BIG_2 - t - Xc_Hnew_BIG_2)
                ch = vcat(ch_startX,ch_t)
            end
...
"""
function do_fitting_recursive(list_of_tbcs ; weights_list = missing, dft_list=missing, kpoints = missing, starting_database = missing,  update_all = false, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, energy_weight = missing, rs_weight=missing,ks_weight=missing, niters=50, lambda=[0.0,0.0], leave_one_out=false, prepare_data = missing, RW_PARAM=0.0, NLIM = 100, refit_database = missing, start_small = false, fit_to_dft_eigs=false, fit_eam=false, ch_startX = missing, energy_diff_calc = false, gen_add_ham=false, fitting_version = fitting_version_default)
...
function topstuff(list_of_tbcs, prepare_data; weights_list=missing, dft_list=missing, kpoints = [0 0 0; 0 0 0.5; 0 0.5 0.5; 0.5 0.5 0.5], starting_database = missing,  update_all = false, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, energy_weight = missing, rs_weight=missing, ks_weight = missing, niters=50, lambda=[0.0, 0.0], leave_one_out=false, RW_PARAM=0.0001, KPOINTS=missing, KWEIGHTS=missing, nk_max = 0)
...
#    return 
    ...
    return list_of_tbcs,KPOINTS, KWEIGHTS, dft_list, scf, energy_weight, rs_weight, ks_weight, weights_list, NWAN_MAX, NCALC, VALS, VALS0, E_DEN, H1, H1spin, DQ, DQ_EDEN, ENERGY_SMEAR, OCCS, WEIGHTS, ENERGIES, NCOLS_orig, NCOLS, ch, NVAL, NAT , SPIN_MAX, Ys, keep_bool, keep_inds, toupdate_inds, ch_keep, keep_inds_S, toupdate_inds_S, cs_keep
...
#=
function do_fitting_recursive_main(list_of_tbcs, prepare_data; weights_list=missing, dft_list=missing, kpoints = [0 0 0; 0 0 0.5; 0 0.5 0.5; 0.5 0.5 0.5], starting_database = missing,  update_all = false, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, energy_weight = missing, rs_weight=missing, ks_weight = missing, niters=50, lambda=[0.0, 0.0], leave_one_out=false, RW_PARAM=0.0001, KPOINTS=missing, KWEIGHTS=missing, nk_max=0, start_small=false, fit_to_dft_eigs=false, fit_eam=false, optimS = false, top_vars = missing, ch_startX = missing, energy_diff_calc = false)
    ...
    println("NOW, DO RECURSIVE FITTING  Ys $(sum(abs.(Ys)))  Ys_new $(sum(abs.(Ys_new)))")
...
#                    println(" add 2b inds $ind3")
                    NEWX[counter,ind3] = lambda[1]
                else
#                    println("!add 3b inds $ind3")                    
...
#        println("test ", sum(abs.(NEWX * ones(size(NEWX)[2]) - NEWY)))
        ...
        return NEWX, NEWY, energy_counter, CALC_IND
    ...
    function do_iters(chX, NITERS; leave_out=-1)
        ...
        NEWX = zeros(2,2)
        NEWY = zeros(2)
        energy_weight_orig = deepcopy(energy_weight)
        energy_weight = 1.0

        error_energy_previous = 10^15
...
##            println(typeof(VECTS_FITTED))
  ...
  #          println(typeof(leave_out))
            println("construct_newXY")
            @time  NEWX, NEWY, energy_counter, CALC_IND = construct_newXY(VECTS_FITTED, OCCS_FITTED, NCALC, NCOLS, NLAM, ERROR, EDEN_FITTED, leave_out=leave_out)
...
#            save("a.jld", "TOTX", TOTX, "TOTY", TOTY, "CALC_IND", CALC_IND)
...
#                error_new_rs  = sum((X_H * chX .- Y_H).^2)
                ...
                if error_new_energy > error_energy_previous && iters > 5
                    println("reduce mix new $error_new_energy error_energy_previous $error_energy_previous")
                    mix = mix * 0.8
                end
                error_energy_previous = error_new_energy
...
#                println("errors rs     new $error_new_rs old $error_old_rs")
                ...
                for calc = 1:NCALC
                    ...
                    c_calc += 1 
                    ind = (CALC_IND .== calc)
#                    println("size(t) ", size(t), " calc ind ", size(CALC_IND))
                    println("error calc $calc ", sum(t[1:length(CALC_IND)][ind]), " energy ", t[energy_counter[c_calc]])
                    if sum(t[1:length(CALC_IND)][ind]) > 10^5
                        println("WARNING turning off $calc due to large error !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        ERROR[calc] = 1
                    end
                end
...
#                println()
            ...
            if abs(error_new_energy - err_old_en) < 2e-3 && iters >= 15
                println("break")
                break
            else
...
#            err_old_en = error_new_energy
        ...
        println("final error $(sum((NEWX * chX .- NEWY).^2))   ERROR $(sum(ERROR))") 
        ...
        @time chX = TOTX \ TOTY
#        save("b.jld", "TOTX", TOTX, "TOTY", TOTY, "CALC_IND", CALC_IND)
#        println()
        println("final error $(sum((TOTX * chX .- TOTY).^2))")
        ...
        for calc = 1:NCALC
            ...
            ind = (CALC_IND .== calc)
            println("error calc $calc ", sum(t[1:length(CALC_IND)][ind]), " energy ", t[energy_counter[c_calc]])
            #println("error calc $calc ", sum(t[1:length(CALC_IND)][ind]))
        end
=#
        println()
        println("final chX = ", chX)
        println()
        println("final cs = ", cs)
        
        construct_fitted(chX, solve_scf_mode)
        println()
        println("done final output")
        println()
        println()
...
#        end
        ...
        println("good")
        println(good)
        println("make database optimS $optimS")
...
#    end            
    ...
    if !ismissing(ch_startX)
        println("using ch_startX")
        ch[1:length(ch_startX)] = ch_startX[:]
        ch[length(ch_startX)+1:end] *= 0.5
    end
...
#    ch_lin = ones(size(ch_lin))
    println("ch_lin ", ch_lin)
    
    if ismissing(top_vars)
        list_of_tbcs,KPOINTS, KWEIGHTS, dft_list, scf, energy_weight, rs_weight, ks_weight, weights_list, NWAN_MAX, NCALC, VALS, VALS0, E_DEN, H1, H1spin, DQ, DQ_EDEN, ENERGY_SMEAR, OCCS, WEIGHTS, ENERGIES, NCOLS_orig, NCOLS, ch, NVAL, NAT, SPIN_MAX, Ys, keep_bool, keep_inds, toupdate_inds, ch_keep, keep_inds_S, toupdate_inds_S, cs_keep =
            topstuff(list_of_tbcs, prepare_data; weights_list=weights_list, dft_list=dft_list, kpoints = kpoints, starting_database = starting_database,  update_all = update_all, fit_threebody=fit_threebody, fit_threebody_onsite=fit_threebody_onsite, do_plot = do_plot, energy_weight = energy_weight, rs_weight=rs_weight, ks_weight = ks_weight, niters=niters, lambda=lambda,  leave_one_out=false, RW_PARAM=RW_PARAM, KPOINTS=KPOINTS, KWEIGHTS=KWEIGHTS, nk_max = nk_max)
    else
        list_of_tbcs,KPOINTS, KWEIGHTS, dft_list, scf, energy_weight, rs_weight, ks_weight, weights_list, NWAN_MAX, NCALC, VALS, VALS0, E_DEN, H1, H1spin, DQ, DQ_EDEN, ENERGY_SMEAR, OCCS, WEIGHTS, ENERGIES, NCOLS_orig, NCOLS, ch, NVAL, NAT, SPIN_MAX, Ys, keep_bool, keep_inds, toupdate_inds, ch_keep, keep_inds_S, toupdate_inds_S, cs_keep = top_vars
    end
    Ys = Ys_new + Xc_Snew_BIG
...
#                    println("valsf ", size(VALS_FITTED), " " , size(VALS_FITTED[c,1:nk,1:nw, 1:tbc.tb.nspin]))
                    ...
                    dq_new,dq_eden_new=get_dq(tbc, eden)
#                    println("EDEN DDDDDDDDDD ", eden)
...
#                h1 = (h1 + H1[c,1:nw,1:nw]) / 2.0   #mixing
            ...
            elseif scf
                ...
                h1 = (h1 + H1[c,1:nw,1:nw]) / 2.0   #mixing
                dq = (dq + DQ[c,1:tbc.crys.nat]) / 2.0
                dq_eden = (dq_eden + DQ_EDEN[c,1:nw]) / 2.0
                h1spin = (h1spin + H1spin[c,:,1:nw,1:nw]) / 2.0
                H1[c,1:nw,1:nw] =  h1 
                DQ[c,1:tbc.crys.nat] = dq
                DQ_EDEN[c,1:nw] = dq_eden
...
#    function construct_newXY(VECTS_FITTED::Array{Complex{Float64},4}, OCCS_FITTED::Array{Float64,3}, ncalc::Int64, ncols::Int64, nlam::Int64, ERROR::Array{Int64,1}; leave_out=-1)
    function construct_newXY(VECTS_FITTED, OCCS_FITTED::Array{Float64,4}, ncalc::Int64, ncols::Int64, nlam::Int64, ERROR::Array{Int64,1}, EDEN_FITTED::Array{Float64,3}; leave_out=-1, chX = missing)
            ...
            for spin = 1:list_of_tbcs[calc].nspin
                for k = 1:nk
                    ...
                    for i = 1:nw
                        ...
                        NEWX[counter, :] = vals_test_other[i,:] .* WEIGHTS[calc, k, i, spin] * w_special
                        X_TOTEN[:] +=   vals_test_other[i,:] .* (KWEIGHTS[calc][k] * OCCS_FITTED[calc,k,i, spin])  #* list_of_tbcs[calc].nspin
        ...
        if energy_diff_calc
            if indmin != 0 && energy_min_arr_ind != 0
                for (calc,ind) in enumerate(EDIFF_IND)
                    NEWX[ind, :] = ( NEWX[ind, :] / NAT[calc] - NEWX[energy_min_arr_ind, :]/NAT[indmin] ) * energy_diff_weight  * weights_list[calc]
                    NEWY[ind] = ( NEWY[ind] / NAT[calc] - NEWY[energy_min_arr_ind] /NAT[indmin] ) * energy_diff_weight * weights_list[calc]
                end
            end
        end
...
#        println("test ", sum(abs.(NEWX * ones(size(NEWX)[2]) - NEWY)))
    ...
    function do_iters(chX, NITERS; leave_out=-1)
        ...
        NEWY = zeros(2)
        energy_weight_orig = deepcopy(energy_weight)
        ks_weight_orig = ks_weight
        kw_weight = max(0.1, ks_weight_orig)
...
##            println(typeof(VECTS_FITTED))
  ...
  #          println(typeof(leave_out))
            println("construct_newXY")
            @time  NEWX, NEWY, energy_counter, CALC_IND = construct_newXY(VECTS_FITTED, OCCS_FITTED, NCALC, NCOLS, NLAM, ERROR, EDEN_FITTED, leave_out=leave_out, chX = chX)
...
=#
        ...
        if gen_add_ham
            ch_mean = missing
            ch_arr = missing
            println("generate_additional_hams")
            @time ch_mean, ch_arr = generate_additional_hams(TOTX ,  TOTY, threebody_inds, deepcopy(chX2), lam = lambda)
                ...
                if update_all == false && !ismissing(starting_database)
                    for key in keys(databaseX)
                        if key in keys(starting_database)
                            databaseX[key] = starting_database[key]
                        end
                    end
                end
...
function get_electron_density(tbc, kpoints, kweights, vects, occ, S; nel = missing)
...
#    if !ismissing(nel)
#        electron_den = electron_den  .- (sum(electron_den) -  nel*0.5*tbc.nspin) / length(electron_den)
#    end
...
#    println(ismissing(SPIN))
    ...
    @time for (tbc, kpoints, kweights, d, spin ) in zip(list_of_tbcs, KPOINTS, KWEIGHTS, dft_list, SPIN)
        ...
        if !ismissing(d)
            nval = dft.bandstruct.nelec
        elseif sum(tbc.eden) > 1e-10
            nval = sum(tbc.eden) / size(tbc.eden)[1]
        else
            nval = nval
        end
...
#    return 
    ...
    return ENERGIES, WEIGHTS, OCCS, VALS, H1, DQ, DQ_EDEN, E_DEN, H1spin, VALS0, ENERGY_SMEAR, NWAN_MAX, SPIN_MAX, NAT_MAX, NCALC, NVAL, NAT
...
#            VECTS_FITTED = Dict()
    ...
    @time ENERGIES, WEIGHTS, OCCS, VALS, H1, DQ, DQ_EDEN, E_DEN, H1spin, VALS0, ENERGY_SMEAR, NWAN_MAX, SPIN_MAX, NAT_MAX, NCALC, NVAL, NAT =     prepare_rec_data( list_of_tbcs, KPOINTS, KWEIGHTS, dft_list, SPIN, ind_BIG, nk_max, fit_to_dft_eigs, scf, RW_PARAM, weights_list)
    ...
    for iter_global = 2:niters_global
        ...
        @time @suppress ENERGIES, WEIGHTS, OCCS, VALS, H1, DQ, DQ_EDEN, E_DEN, H1spin, VALS0, ENERGY_SMEAR, NWAN_MAX, SPIN_MAX, NAT_MAX, NCALC, NVAL, NAT =     prepare_rec_data( list_of_tbcs, KPOINTS, KWEIGHTS, dft_list, SPIN, ind_BIG, nk_max, fit_to_dft_eigs, scf, RW_PARAM, weights_list)
