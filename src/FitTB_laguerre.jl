...
"""
function do_fitting(list_of_tbcs; dft_list = missing, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, starting_database=Dict(), fit_eam=false)

    ret = do_fitting_linear(list_of_tbcs; dft_list=dft_list, fit_threebody=fit_threebody, fit_threebody_onsite = fit_threebody_onsite, do_plot = do_plot, starting_database=starting_database, fit_eam=fit_eam )
    return ret[1]
...
function prepare_for_fitting(list_of_tbcs; kpoints = missing, dft_list = missing, fit_threebody=false, fit_threebody_onsite=false, starting_database=missing, refit_database=missing, factor_dict = missing, fit_umat = false, fit_eam = false)
    ...
    println("prepare_for_fitting $fit_eam !!!!!!!!!!!!!!!!")
    #sleep(10)
...
#            println(keepind)
        ...
        sgn, dat, SS, TT, atom_trans = get_symmetry(tbc.crys, verbose=false);
        push!(SYM_INFO, [SS, atom_trans, orb2ind])
...
#            println(h_onsite2)
        ...
        for key in keys(twobody_arrays)
            if !( (key,2) in KEYS)
                ...
                if fit_umat
                    UIND[(key,2)] = 1+unum:unum+twobody_arrays[key][3].sizeU
                    unum += twobody_arrays[key][3].sizeU
                else
                    UIND[(key,2)] = []
                end
...
#####################################
    ...
    if !ismissing(starting_database )
        ...
        ch_start, cs_start, cu_start = extract_database(starting_database, nh, ns, KEYS, HIND, SIND, UIND = UIND, nu = nu)
...
#                cs_lin[i] = cs_start[i]
        ...
        if fit_umat
            for i = 1:nu
                if abs(cu_start[i] + 999999.0) < 1e-7
                    cu_start[i] = cu_lin[i]
                    #                cs_lin[i] = cs_start[i]
                    push!(toupdate_inds_U, i)
                else
                    push!(keep_inds_U, i)
                end
            end
        ...
        cu_keep = cu_start[keep_inds_U]
        

        cs = deepcopy(cs_start)
        ch = deepcopy(ch_start)
        cu = deepcopy(cu_start)
...
#            keep_inds = Int64[]
        ...
        toupdate_inds_U = 1:length(cu_lin) #all
        keep_inds_U = Int64[]
    ...
    keepdata = Any[ch_keep, keep_inds, toupdate_inds, cs_keep, keep_inds_S, toupdate_inds_S, cu_keep, keep_inds_U, toupdate_inds_U]
    
    hnum_new = length(toupdate_inds)
    snum_new = length(toupdate_inds_S)
    unum_new = length(toupdate_inds_U)
...
#    Y_HnewOrth_BIG = Float64[]
    ...
    ind_BIG_U = zeros(Int64, size(INDVEC)[1])
...
#        println("first part $c")
        ...
        X_U_temp = zeros(tbc.crys.nat, unum) #old sizes
        X_U_new = zeros(tbc.crys.nat, unum_new) #old sizes
        
        Xsc = zeros(size(svec)[1], 1)
        Xhc = zeros(size(hvec[1])[1], 1)
        Xuc = zeros(tbc.crys.nat, 1)
...
#                X_S[rind,sind] = arr2[key][2]
                ...
                if fit_umat
                    ...
                    println(arr2[key][4])
                    println("uind $uind")
                    println("size X_U_temp ", size(X_U_temp))
                    X_U_temp[:,uind] = arr2[key][4]
                end
...
#                X_U[rind,:] = X_U_temp[:,toupdate_inds_U]
            end
...
function do_fitting_linear(list_of_tbcs; kpoints = missing, dft_list = missing,  fit_threebody=true, fit_threebody_onsite=true, do_plot = false, starting_database=missing, mode=:kspace, return_database=true, NLIM=120, refit_database=missing, factor_dict = missing, rind = missing, fit_umat = false, fit_eam = false)
    ...
    #   rind = 1:l
        println("sum abs XYU ", sum(abs.(X_Hnew_BIG[rind,:])), " ", sum(abs.(Float64.(Y_Hnew_BIG[:,1] - Xc_Hnew_BIG)[rind])))
...
#    println("SYM_INFO ", SYM_INFO)
    ...
    return database, ch, cs, X_Hnew_BIG, Xc_Hnew_BIG, Xc_Snew_BIG, X_H, X_Snew_BIG, Y_H, Y_S, HON, ind_BIG, KEYS, HIND, SIND, DMIN_TYPES, DMIN_TYPES3, keepind, keepdata, Y_Hnew_BIG, Y_Snew_BIG, YS_new, cs , ch_refit, SPIN, threebody_inds, rind, X_Unew_BIG_list, Xc_Unew_BIG_list, UIND, SYM_INFO
...
"""
function make_database(ch, cs,  KEYS, HIND, SIND, DMIN_TYPES, DMIN_TYPES3; scf=false, starting_database=missing, tbc_list=missing, cu=missing, UIND = missing, fit_eam = false)
...
#            end
    ...
    for key in KEYS
        ...
        if !ismissing(UIND) 
            coef = make_coefs(atomkey,dim, datH=ch[hind], datS=cs[sind], min_dist=dmin, dist_frontier = frontier, datU=cu[uind], use_eam=fit_eam)
        else
...
"""
function extract_database(database_old,nh,ns, KEYS, HIND, SIND; nu = 0, UIND = missing)
    ch = zeros(nh) .- 999999.0
    cs = zeros(ns) .- 999999.0
    cu = zeros(nu) .- 999999.0
                    ...
                    if fit_to_dft_eigs
                        ...
                        for k2 in 1:d.bandstruct.nks
                            if sum(abs.(d.bandstruct.kpts[k2,:] - kpoints[k,:]) ) < 1e-5
                                ...
                                VALS[c, k,1+nval_int:n, spin] = VALS[c, k,1+nval_int:n,spin] 
...
#        println(X_Unew_BIG_list)
#        println("1")
#        println(X_Unew_BIG_list[1])
#        println("size ", size(X_Unew_BIG_list))
        NCOLS_U = size(X_Unew_BIG_list[1])[2]
    end
...
#    c_umat = ones(NCOLS_U) * 0.0
    ...
    #fix testing
    for (c,tbc) in enumerate(list_of_tbcs)
#        println("h1 before ", tbc.tb.h1)
#        println("gamma before ", tbc.gamma)
        gamma,_ = electrostatics_getgamma(tbc.crys)
        ec_old = ewald_energy(tbc)
        tbc.gamma = gamma
#        println("gamma after ", tbc.gamma)
        h1,dq,dq_eden = get_h1(tbc)
        tbc.tb.h1 = h1
#        println("h1 after ", tbc.tb.h1)
        nw = tbc.tb.nwan
        H1[c,1:nw,1:nw] = h1
#        println("H1 after ", H1[1,:,:])
        ec_new = ewald_energy(tbc)
...
#        println(gamma)
        ...
        for i = 1:crys.nat
            ...
            for j = 1:crys.nat
                ...
                for c1 = o1:o1+nw1-1
                    for c2 = o2:o2+nw2-1
                        h1_umat[c1,c2] += 0.5*(epsilon[i] + epsilon[j])
                    end
                end
                o2 += nw2
            end
...
#    println("SCF4 is $scf")
    ...
    function construct_fitted(ch, solve_self_consistently = false)
        ...
        function get_eigen(h1_in, h1spin_in, nspin, h_umat)
...
#                            SS[j,i] = Y_Snew_BIG[row1-1 + (k-1)*N + ind] - im*Y_Snew_BIG[row1-1 + (k-1)*N + ind + nk * N]
            ...
            h1_umat = zeros(nw, nw)
            if fit_umat
                energy_umat, h1_umat = get_h1_umat(dq, tbc.crys, Xc_Unew_BIG_list, X_Unew_BIG_list , c_umat, c, h1_umat)
            else
                h1_umat .= 0.0
                energy_umat = 0.0
            end
...
#            println()
            ...
            for c_scf = 1:niter_scf
                ...
                get_eigen(h1, h1spin, tbc.tb.nspin, h1_umat)
#                println("$c_scf aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa $(sum(abs.(h1))) $(VALS_FITTED[1])")               
...
#                    println("valsf ", size(VALS_FITTED), " " , size(VALS_FITTED[c,1:nk,1:nw, 1:tbc.tb.nspin]))
                    ...
                    eden_new, h1_new, dq_new, h1spin_new = get_electron_density(tbc, kpoints, kweights, VECTS_FITTED[c], occs, Smat)         #updated h1
#                    println("dq_new $dq_new  dq  $dq")
...
#                    dq = dq*(1-mix) + dq_new * mix
                    ...
                    #    h1spin = h1spin*(1-mix) + h1spin_new * mix
                        ...
                        tbc.eden[:] = eden
                        energy_charge = ewald_energy(tbc)
#                        println("energy_charge $energy_charge dq $dq $(sum(abs.(tbc.gamma)))")
#                        if c_scf == 1
#                            println("crys")
#                            println(tbc.crys)
#                            println("gamma")
...
#                        end
                    ...
                    energy_new = energy_charge + energy_band + energy_smear + energy_magnetic + energy_umat
                    #println("c_scf $c_scf energgy_new $energy_new ", [energy_charge , energy_band , energy_smear , energy_magnetic], "dq ", dq)
...
#        println("solve_self_consistently $solve_self_consistently")
        ...
        for (tbc, kpoints, kweights, dft, sym_info) in zip(list_of_tbcs, KPOINTS, KWEIGHTS, dft_list, SYM_INFO)
            ...
            nval = NVAL[c]
#            println("NVAL $c  $nval  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
...
#                println("h1_in ", h1_in)
#                println()
#                println("h_umat (eigen) ", h_umat)
#                println()
#                println("htot (eigen) ", h1_in +  h_umat)
#                println()
                vals = zeros(Float64, nw)
                vects = zeros(Complex{Float64}, nw, nw)
                VECTS = zeros(Complex{Float64}, nspin, nk, nw, nw)
...
#                            println("vals !!!!!  ", vals)
                        ...
                        VECTS[spin, k,1:nw,1:nw] = vects
                        VALS_FITTED[c,k,1:nw, spin] = real(vals)
#                        println("new vals[1] c $c k $k $(vals[1])")
                        #                        println("VALS_FITT $spin $k ", real.(vals))
                        #                    VALS_FITTED[c,k,1:nw] = real.(diag(vects'*H*vects))
...
#            println("niter_scf ", niter_scf)
#            println("dq start $dq  dq eden $(dq_eden)")

            #            for c_scf = 1:niter_scf
            conv = true
            eden = reshape(eden, 1, length(eden))
#            println("size eden start ", size(eden))
            for c_scf = 1:niter_scf
...
#                println("$c_scf aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  $(sum(abs.(h1))) $(VALS_FITTED[1])")                
...
#                    println("valsf ", size(VALS_FITTED), " " , size(VALS_FITTED[c,1:nk,1:nw, 1:tbc.tb.nspin]))                                                                                                                                                                             
                    ...
                    #                    eden, h1_new, dq_new = get_electron_density(tbc, kpoints, kweights, VECTS_FITTED[c,:,1:nw,1:nw], occs, Smat)         #updated h1                                                                                                                   
                                  eden_new, h1_new, dq_new, h1spin_new = get_electron_density(tbc, kpoints, kweights, VECTS_FITTED[c], occs, Smat)         #updated h1                                                                                                                                        
#                                  println("size eden_new ", size(eden_new))
                    for spin =1:tbc.tb.nspin
                        SS, atom_trans, orb2ind = sym_info
...
#                    h1 = h1*(1-mix) + h1_new * mix                                                                                                                                                                                                                                          
#                    dq = dq*(1-mix) + dq_new * mix                                                                                                                                                                                                                                          
#                                  println("before new ", size(eden_new), " old ", size(eden))

                                  eden = eden*(1-mix) + eden_new * mix
                                  eden =reshape(eden, 1, length(eden))
#                                  println("after ", size(eden))
                    ...
                    EDEN_FITTED[c, 1:tbc.nspin,1:nw] = eden                                                                                                                                                                                                                                 
                                  ...
                                  h1, dq, dq_eden = get_h1(tbc, reshape(eden, 1, nw))
...
#                    println("c_scf $c_scf energgy_new $energy_new ", [energy_charge , energy_band , energy_smear , energy_magnetic])                                                                                                                                                        
                    #                    println( " scf $c_scf $c ", energy_new+etypes, "    $dq   $energy_charge $energy_band $energy_smear $energy_magnetic")                                                                                                                             
...
#                    mix = 0.0
                    ...
                    #efermi = calc_fermi(VALS_FITTED[c,1:nk,1:nw, 1:tbc.tb.nspin], kweights, nval, 0.01)
                    energy_tmp,  efermi = band_energy(VALS_FITTED[c, 1:nk,1:nw,1:tbc.tb.nspin], kweights, nval, 0.01, returnef=true) 
#                    println("band energy tmp $energy_tmp   efermi $efermi")
                    occs = gaussian.(VALS_FITTED[c,1:nk,1:nw, 1:tbc.tb.nspin].-efermi, 0.01)
#                    println("occs[1] ", occs[1], " ef ", efermi, " vals1 ", VALS_FITTED[1])
...
#                    println("eden_new $eden_new")
                    ...
                    #println("occs[1] v2 ", occs[1], " ef ", efermi, " vals1 ", VALS_FITTED[1])                    
                    s1 = sum(occs .* VALS0_FITTED[c,1:nk,1:nw, 1:tbc.tb.nspin], dims=[2,3])
                    energy_band = sum(s1 .* kweights) #/ tbc.tb.nspin
                    #println("band energy  $energy_tmp   efermi $efermi")
                    #for tk = 1:nk
                    #    println("k vals0 $tk $(VALS0_FITTED[c,tk,1:nw, 1][:])  occs $(occs[tk,:,1][:])  kw $(kweights[tk])")
                    #end

#                    println("c_scf $c_scf mix $mix energy_band $energy_band dq  $dq_eden_new ")                    
                    #                    println("energy_band ", occs[1] , " " ,  VALS0_FITTED[1], " ", kweights[1])
...
#                    h1 = h1*(1-mix) + h1_new * mix
#                    println("dq $dq dq_new $dq_new")
                    #dq = dq*(1-mix) + dq_new * mix
                    #prinrintln("dq_eden"
#                    println("dq_eden $dq_eden dq_eden_new $dq_eden_new mix $mix")                    
...
#                    if solve_self_consistently == true
                    #    h1 = h1*(1-mix) + h1_new * mix + h1_umat
                    #    h1spin = h1spin*(1-mix) + h1spin_new * mix
                    if scf
                        energy_charge = ewald_energy(tbc, dq, dq_eden)
                        if tbc.tb.scfspin
...
#            println("get_eigen end h1_umat $h1_umat")
            ...
            energy_tmp,  efermi = band_energy(VALS_FITTED[c, 1:nk,1:nw,1:tbc.tb.nspin], kweights, nval, 0.01, returnef=true)             
            #            efermi = calc_fermi(VALS_FITTED[c,1:nk,1:nw,1:tbc.tb.nspin], kweights, nval, 0.01)
#            println("occs[1] v3 ", occs[1], " ef ", efermi, " vals1 ", VALS_FITTED[1])
            occs = gaussian.(VALS_FITTED[c,1:nk,1:nw,1:tbc.tb.nspin].-efermi, 0.01)
#            println("occs[1] v4 ", occs[1], " ef ", efermi, " vals1 ", VALS_FITTED[1])                    
            
            OCCS_FITTED[c,1:nk,1:nw,1:tbc.tb.nspin] = occs
#            println("occs new c $c nk1 $(OCCS_FITTED[c,1,1:nw,1])")
            ...
            if scf
                energy_charge = ewald_energy(tbc, dq, dq_eden)
#                println("ewald_energy #energy_charge $dq_eden")
            else
                energy_charge = 0.0
            end
            ENERGY_CHARGE[c] = energy_charge
...
#    println("SCF5 is $scf")
    ...
    function construct_newXY(VECTS_FITTED, OCCS_FITTED::Array{Float64,4}, ncalc::Int64, ncols::Int64, ncols_U::Int64, nlam::Int64, ERROR::Array{Int64,1}, EDEN_FITTED::Array{Float64,3}; leave_out=-1)
        ...
        NEWX = zeros(counter + nlam, ncols + ncols_U)
        ...
        for calc = 1:ncalc
            ...
            if fit_umat
                for a = 1:nat
                    energy_umat_fixed += 0.5 * Xc_Unew_BIG_list[calc][a] * DQ[calc, a]^2
...
#            println("Y_TOTEN subtract $Y_TOTEN ", [energy_charge , etypes , energy_smear , energy_magnetic , energy_umat_fixed  ], "fit_umat ", fit_umat)
...
#        println("size chX do_iters ", size(chX))
        ...
        for iters = 1:NITERS #inner loop
            ...
            println("DOING ITER $iters ------------------------------------------------------------- mix $mix")
#            println("chX ", chX)
#            println("c_umat ", c_umat)
#            println("DQ ", DQ)
            ...
            println("done construct_newXY ", sum(abs.(NEWX)), " " , sum(abs.(NEWY)), " size ", size(NEWX))
            ...
            println("prepare fit ks_weight $ks_weight rs_weight $rs_weight")
            @time begin
                TOTX = NEWX
                TOTY = NEWY
...
#                println("SUM TOTX ", sum(abs.(TOTX)))
                ...
                if ks_weight > 1e-5 || iters <= 3
                #if false
                    if iters <=3 && ks_weight < 0.05
                        ks_weight = 0.05
                    end
                    println("sum abs X_Hnew_BIG ", sum(abs.(X_Hnew_BIG)) , "  " , sum(abs.(Y_Hnew_BIG)), " size ", size(X_Hnew_BIG))
                    l = length(Y_Hnew_BIG[:,1]) #randomly sample Hk to keep size reasonable
                    if l > 5e5
                        frac = 5e5 / l
                        println("randsubseq $frac")
                    ...
                    TOTX = [hcat(X_Hnew_BIG[rind,:]*ks_weight, zeros(length(rind), NCOLS_U));TOTX]
...
#                error_old_energy  = sum((NEWX * chX .- NEWY).^2)
            ...
            #println("ch_old ", ch_old)
            #println()
            #println("old ", sum(abs.(TOTX * ch_old - TOTY)))
            #println()
            #println("old ", [TOTX * ch_old   TOTY TOTX * ch_old  - TOTY   ])
            #println()
            #println("fit")
            @time ch_new = TOTX \ TOTY
            println("ch_new ", sum(abs.(ch_new)), " ", ch_new[1:5])
            println("err XXXXXXXXXXXX ", sum(abs.(TOTX*ch_new - TOTY)) , " " , sum(abs.(NEWX*ch_new - NEWY)), " ", sum(abs.(TOTX)), " " , sum(abs.(TOTY)))
            println("size ", size(TOTX), " ", size(TOTY))            
            #println("ch_new ")
            #println(ch_new)
...
#                println("errors energy new $error_new_energy")
...
#        end
        ...
        if returndatabase == false
            ...
            database = make_database(chX2, csX2,  KEYS, HIND, SIND,DMIN_TYPES,DMIN_TYPES3, scf=scf, starting_database=starting_database, tbc_list = list_of_tbcs[good], cu=cu_big, UIND=UIND, fit_eam=fit_eam )
        end
    ...
    function construct_fitted(ch, solve_self_consistently = false)
        ...
        for (tbc, kpoints, kweights, dft) in zip(list_of_tbcs, KPOINTS, KWEIGHTS, dft_list)
            ...
            if scf
                ...
                dq = deepcopy(DQ[c,1:tbc.crys.nat])
                dq = zeros(tbc.crys.nat)
                dq_eden = deepcopy(DQ_EDEN[c,1:tbc.tb.nwan])
            else
                h1 = zeros(Float64, nw, nw)
                dq = zeros(tbc.crys.nat)
            end
...
#                    println( " scf $c_scf $c ", energy_new+etypes, "    $dq  dq_eden $dq_eden  $energy_charge $energy_band $energy_smear $energy_magnetic")
    ...
    function do_iters(chX, NITERS; leave_out=-1)
        ...
        for iters = 1:NITERS #inner loop
            ...
            if scf
                if iters == 2 println("TURNING ON SCF SOLVE!!!!!!!!!!!!!!!!!!!!!!! ") end
                println()
                solve_scf_mode = true
            end
...
###############################################################################OLD
function do_fitting_recursive_main_old2(list_of_tbcs, prepare_data; weights_list=missing, dft_list=missing, kpoints = [0 0 0; 0 0 0.5; 0 0.5 0.5; 0.5 0.5 0.5], starting_database = missing,  update_all = false, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, energy_weight = missing, rs_weight=missing, ks_weight = missing, niters=50, lambda=0.0, leave_one_out=false, RW_PARAM=0.0001, KPOINTS=missing, KWEIGHTS=missing, nk_max=0, start_small=false, fit_to_dft_eigs=false, returnstuff=false, topstuff=missing, returndatabase=true, cs_start=missing, fit_umat = fit_umat, gen_add_ham = false, fit_eam=false)
...
#            println("x ", tbc.tb.h1)
            ...
            H1[c,1:nw, 1:nw] = tbc.tb.h1
            DQ[c,1:tbc.crys.nat] = get_dq(tbc)[1]
#            println("SIZE tbc.tb.h1spin ", size(tbc.tb.h1spin))
            H1spin[c,:,1:nw, 1:nw] = tbc.tb.h1spin
        end
...
#    return database
    ...
    function do_fitting_recursive_main_old(list_of_tbcs, prepare_data; weights_list=missing, dft_list=missing, kpoints = [0 0 0; 0 0 0.5; 0 0.5 0.5; 0.5 0.5 0.5], starting_database = missing,  update_all = false, fit_threebody=true, fit_threebody_onsite=true, do_plot = false, energy_weight = missing, rs_weight=missing, ks_weight = missing, niters=50, lambda=0.0, leave_one_out=false, RW_PARAM=0.0001, KPOINTS=missing, KWEIGHTS=missing, nk_max=0, start_small=false, fit_to_dft_eigs=false, returnstuff=false, topstuff=missing, returndatabase=true, cs_start=missing, fit_umat = fit_umat, gen_add_ham = false, fit_eam=false)
...
#            println("dq end $dq  dq eden $(dq_eden)")
=#
